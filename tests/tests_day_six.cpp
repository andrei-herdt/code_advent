...
TEST(day_six, part_one) {
    ...
    ASSERT_EQ(closed_ids.size(), 2);
    ASSERT_THAT(closed_ids, testing::ElementsAre(3, 4));
    ...
    ASSERT_EQ(closed_ids_count[3], 9);
    ASSERT_EQ(closed_ids_count[4], 17);
}

TEST(day_six, part_one_data) {
    ifstream input_file;
